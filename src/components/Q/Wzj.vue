...
export default {
  ...
  methods: {
    ...
    async Info (row) {
      ...
      delete row.filed
      this.insert[0] = row
      const res = await postData('/special/insert', this.insert)
      if (res.code === 200) {
    ...
    async gozj () {
      ...
      const res = await postData('/special/access?pdId=' + this.addFrom.pdId)
      if (res.code === 200) {
    ...
    sumbit () {
      ...
      this.$refs.addRef.validate(async (valid) => {
        if (!valid) return
        if (valid) {
          const res = await postData('/special/notPass', qs.stringify(this.addFrom))
          if (res.code === 200) {
