<template>
<!-- 车间抽检---抽检 -->
<div>
      <el-card class="box-card" shadow="always">
         <el-form :model="workCj" :inline="true" class="demo-form-inline"  label-width="100px">
            <el-form-item label="批次号" prop="batchNumber">
               <el-input autocomplete="off" v-model="workCj.batchNumber"></el-input>
            </el-form-item>
             <el-form-item label="产品代号" prop="productCode">
                <el-input autocomplete="off" v-model="workCj.productCode"></el-input>
            </el-form-item>
              <el-form-item label="零部件代号" prop="componmentCode">
                <el-input autocomplete="off" v-model="workCj.componmentCode"></el-input>
            </el-form-item>
              <el-form-item label="零部件名称" prop="componmentName">
                <el-input autocomplete="off" v-model="workCj.componmentName"></el-input>
            </el-form-item>
             <el-form-item label="顺序号" prop="processId">
               <el-input autocomplete="off" v-model="workCj.processId"></el-input>
            </el-form-item>
             <el-form-item label="工序名称" prop="processName">
               <el-input autocomplete="off" v-model="workCj.processName"></el-input>
            </el-form-item>
             <el-form-item label="工序代号" prop="processCode">
               <el-input autocomplete="off" v-model="workCj.processCode"></el-input>
            </el-form-item>
        </el-form>
        <el-table :data="data" border style="width: 100%" >
        <el-table-column prop="pdId" label="序号" :width="50" align="center" v-if="data[0].istrue"></el-table-column>
           <template v-for="(item,index) in testCols">
                <el-table-column :prop="item.filed" :label="item.lable" :key="index"  align="center" :width="250">
                    <template slot-scope="scope">
                        <!-- {{scope.column}} -->
                        <el-input size="small" v-if="item.qualitative===0" v-model="scope.row[item.filed]"  placeholder="请输入内容"  @change="handleEdit(scope.$index, scope.row)"></el-input>
                        <el-radio-group  v-if="item.qualitative===1" v-model="scope.row[item.filed]">
                              <el-radio  label="合格"></el-radio>
                              <el-radio label="不合格"></el-radio>
                        </el-radio-group>
                    </template>
                </el-table-column>
            </template>
    <el-table-column  label="操作" align="center">
         <template slot-scope="scope">
             <el-button type="success" size="mini" @click="Info(scope.row)">提 交</el-button>
        </template>
    </el-table-column>
</el-table>
<div style="margin-left: 50%">
    <i class="el-icon-circle-plus-outline" @click="addmembers"></i>
    <i class="el-icon-delete" @click="deleteItem(row, index)"></i>
</div>
   <div style="margin-left: 80%">
     <el-button type="warning"  size="mini" @click="dialogTableVisible = true">不合格发起</el-button>
     <el-button type="success"  size="mini" @click="gocj()">抽检通过</el-button>
      </div>
    </el-card>
    <el-dialog title="不合格发起" :visible.sync="dialogTableVisible"  width="50%">
       <el-form ref="addRef" :model="addFrom" :inline="true" class="demo-form-inline"  label-width="120px" style="margin-left: 200px" >
           <el-form-item label="id" prop="pdId" v-show="false">
             <el-input v-model="addFrom.pdId"></el-input>
            </el-form-item>
            <el-form-item label="不合格审理单号" prop="codeNumber">
             <el-input v-model="addFrom.codeNumber"></el-input>
            </el-form-item><br>
             <el-form-item label="不合格原因" prop="reason">
             <el-input type="textarea" v-model="addFrom.reason"></el-input>
            </el-form-item>
        </el-form>
          <span slot="footer" class="dialog-footer">
    <el-button @click="dialogTableVisible = false">取 消</el-button>
    <el-button type="primary" @click="sumbit()">提 交</el-button>
  </span>
</el-dialog>
</div>
</template>

<script>
import { postData } from '../../api/index'
import Q from '../Q/FactoryZJ'
export default {
  components: {
    Q
  },
  data () {
    return {
      // 表项（头）
      testCols: [
        {lable: ''}
      ],
      // 表数据
      data: [
        {filed: ''}
      ],
      istrue: true,
      workCj: {
        batchNumber: '',
        productCode: '',
        componmentCode: '',
        componmentName: '',
        processId: '',
        processName: '',
        processCode: ''
      },
      addFrom: {
        pdId: '',
        codeNumber: '',
        reason: ''
      },
      insert: [],
      editDialog: false,
      dialogTableVisible: false,
      formLabelWidth: '120px',
      tableData: [],
      pageNum: 1, // 当前页码
      pageSize: 10, // 每页条数
      total: 1
    }
  },
  created () {},
  mounted () {
    this.getData()
  },
  watch: {},
  computed: {},
  methods: {
    // 抽检通过
    async gocj () {
      this.addFrom.pdId = this.testCols[0].pdId
      const res = await postData('/casul/pass?pdId=' + this.addFrom.pdId)
      if (res.code === 200) {
        this.$message.success(res.msg)
      } else {
        this.$message.error(res.msg)
      }
    },
    sumbit () {
      this.addFrom.pdId = this.testCols[0].pdId
      var qs = require('querystring')
      this.$refs.addRef.validate(async (valid) => {
        if (!valid) return
        if (valid) {
          const res = await postData('/casul/notPass', qs.stringify(this.addFrom))
          if (res.code === 200) {
            this.dialogTableVisible = false
            this.$message.success(res.msg)
            this.$refs.addRef.resetFields()
          } else {
            this.$message.error(res.msg)
          }
        }
      })
    },
    // 删除一行
    deleteItem (row, index) {
      this.data.splice(index, 1)
    },
    // 打开增加一行
    addmembers () {
      var member = this.data
      console.log(member)
      //   var length = member.length
      this.data.push(
        {
        //   id: parseInt(length),
          filed: ''
        })
    },
    handleEdit (val) {
      console.log(val)
    },
    // 提交按钮
    async Info (row) {
      row.pdId = this.testCols[0].pdId
      delete row.filed
      this.insert[0] = row
      const res = await postData('/casul/insert', this.insert)
      if (res.code === 200) {
        this.$message.success('提交成功！')
      } else {
        this.$message.error('提交失败！')
      }
    },
    // 获取路由传参的值
    async getData () {
      var _this = this
      this.workCj = JSON.parse(this.$route.query.data)
      this.testCols = JSON.parse(this.$route.query.res)
      var dataList = JSON.parse(this.$route.query.res1) //   var dataList = this.testDatas
      dataList.forEach(function (item, index) {
        if (item.lable !== '序号') {
          for (let row of _this.testCols) {
            _this.$set([item], '' + row.filed + '', '')
            _this.pdId = row.pdId
          }
        }
      })
    }
  }
}
</script>
<style lang="less" scoped>
.box-card{
    margin-left: -10px;
    width: auto;
    height: auto;

}
/deep/.el-table th {
    padding: 5px 0;
    background-color: #EBEEF5;
}
/deep/ .el-input__inner {
    height: 30px;
    width: 209px;
}
/deep/ .el-input__inner {
    height: 30px;
    width: 209px;
}
     /deep/.el-select .el-input__inner {
  width: 207px;

     }
</style>
