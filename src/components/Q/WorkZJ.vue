      ...
      <el-card class="box-card" shadow="always">
            <el-form :model="form" :inline="true" class="demo-form-inline"  label-width="100px">
              <el-form-item label="批次号" prop="batchNumber">
               <el-input autocomplete="off" placeholder="全部" v-model="form.batchNumber" clearable></el-input>
            </el-form-item>
             <el-form-item label="零部件代号" prop="componmentCode">
                 <el-input autocomplete="off" placeholder="全部" v-model="form.componmentCode" clearable></el-input>
            </el-form-item>
        ...
        <el-table :data="tableData" style="width: 96%"  tooltip-effect="dark"  @selection-change="handleSelectionChange" border>
          ...
          <el-table-column label="序号" width="50" prop="ptId" align="center"> </el-table-column>
           ...
           <el-table-column sortable label="零部件名称" width="130" prop="componmentName" align="center"></el-table-column>
          <el-table-column sortable label="顺序号" width="90" prop="processId" align="center"></el-table-column>
          <el-table-column sortable label="工序名称" width="112" prop="processName" align="center"> </el-table-column>
...
export default {
  ...
  methods: {
    // 查询表格全部
    async getWzj () {
      const res = await getData('/special/selectAll', this.queryWzj)
      if (res.code === 200) {
    ...
    // 搜索按钮
    async getGo () {
      const res = await getData('/special/doSelect', this.form)
      if (res.code === 200) {
    ...
    async goZJ (index, row, ptId) {
      const res = await getData('/special/check?ptId=' + row.ptId)
      const res1 = await getData('/special/check?ptId=' + row.ptId)
