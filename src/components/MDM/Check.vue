...
<div>
     <el-card class="box-card" shadow="always">
        ...
        <el-table :data="tableData" style="width: 100%"  tooltip-effect="dark"  @selection-change="handleSelectionChange" border>
          ...
          <el-table-column sortable label="工艺路线" width="110" prop="rId" align="center"> </el-table-column>
           <el-table-column sortable label="检验顺序" width="100" prop="sequenceId" align="center"> </el-table-column>
          <el-table-column sortable label="工艺参数/检验要求" width="160" prop="examParam" align="center"> </el-table-column>
          <el-table-column sortable label="检验类型" width="100" prop="examType" align="center"> </el-table-column>
   ...
   <el-pagination background
    ...
    <!-- 点击打开修改对话框 -->
    <el-dialog title="请修改工艺检验信息" :visible.sync="editDialog" width="75%">
    ...
    <el-input v-model="editFrom.processName"></el-input>
     ...
     <el-form-item label="工序代号" prop="processCode">
      ...
      <el-select  v-model="editFrom.carftType"  clearable placeholder="请选择工艺类型" @change="getitem2">
          <el-option v-for="item in styles" :key="item.carftId" :label="item.carftType" :value="item.carftId"></el-option>
      </el-select>
    </el-form-item>
      <el-form-item label="检验顺序" prop="sequenceId">
      <el-input v-model="editFrom.sequenceId"></el-input>
    </el-form-item>
     <el-form-item label="检验要求"  prop="examParam">
     <el-input v-model="editFrom.examParam"></el-input>
    </el-form-item>
     <el-form-item label="检验类型" prop="examType">
      <el-select  v-model="editFrom.examType"  clearable placeholder="请选择检验类型" @change="getitem3">
          <el-option v-for="item in style" :key="item.examTypeId" :label="item.examTypeName" :value="item.examTypeId"></el-option>
      </el-select>
    </el-form-item>
     <el-form-item label="定性或着定量" prop="isQualitative">
    <el-select  v-model="editFrom.isQualitative"  clearable placeholder="请选择定性或着定量" @change="getitem4">
 ...
 <!-- 点击打开增加对话框 -->
    <el-dialog title="请增加工艺检验信息" :visible.sync="addDialog" width="75%">
  <el-form :model="addFrom" ref="addRef" :inline="true" class="demo-form-inline" :label-width="formLabelWidth" :rules="addRules">
    ...
    <el-input v-model="addFrom.processName"></el-input>
     ...
     <el-form-item label="检验要求"  prop="examParam">
     <el-input v-model="addFrom.examParam"></el-input>
    </el-form-item>
     <el-form-item label="检验类型" prop="examType">
      <el-select  v-model="addFrom.examType"  clearable placeholder="请选择检验类型" @change="getitem3">
          <el-option v-for="item in style" :key="item.examTypeId" :label="item.examTypeName" :value="item.examTypeId"></el-option>
      </el-select>
    </el-form-item>
     <el-form-item label="定性或着定量" prop="isQualitative">
    <el-select  v-model="addFrom.isQualitative"  clearable placeholder="请选择定性或着定量" @change="getitem4">
          <el-option v-for="item in isQ" :key="item.isQualitativeId" :label="item.isQualitativeName" :value="item.isQualitativeId"></el-option>
      </el-select>
    </el-form-item>
  </el-form>
...
export default {
  ...
  data () {
    return {
      ...
      addRules: {
        componentName: [
          { required: true, message: '请输入零部件名称，不能为空', trigger: 'blur' }
        ],
        productCode: [
        ...
        sequenceId: [
          { required: true, message: '请输入检验顺序，不能为空', trigger: 'blur' }
        ],
        examParam: [
          { required: true, message: '请输入检验要求,不能为空', trigger: 'blur' }
        ]
      },
      editRules: {
        componentName: [
  ...
  methods: {
    // 获取表格数据
    async getCheck () {
      const res = await getData('/exam/examSelect', this.queryCheck)
      if (res.code === 200) {
    ...
    // 搜索按钮
    async getGo () {
      const res = await getData('/exam/examcheckFind', this.form)
      if (res.code === 200) {
    ...
    editInfo () {
      ...
      this.$refs.editRef.validate(async (valid) => {
        if (!valid) return
        if (valid) {
          const res = await postData('/exam/examUpdate', qs.stringify(this.editFrom))
          console.log(res)
          if (res.code === 200) {
    ...
    // 获取修改下拉框数据
    async getSelects () {
      const res = await getData('/exam/examSelectLists')
      if (res.code === 200) {
        this.code = res.data.componentCode
        this.styles = res.data.carftType
        this.style = res.data.checkType
    ...
    async  getProcess1 () {
      const res = await getData('/exam/examProcessIds?componentCode=' + this.editFrom.componentCode)
      if (res.code === 200) {
    ...
    // 获取下拉框数据
    async getSelect () {
      const res = await getData('/exam/examSelectList')
      if (res.code === 200) {
        this.code = res.data.componentCode
        this.styles = res.data.carftType
        this.style = res.data.checkType
    ...
    async  getProcess () {
      const res = await getData('/exam/examProcessId?componentCode=' + this.addFrom.componentCode)
      if (res.code === 200) {
        this.num = res.data.processId
      }
    },
    ...
    getitem2 (val) {
      this.styles.find(item => item.carftId === val)
    },
    getitem3 (val) {
      this.style.find(item => item.examTypeId === val)
    },
    getitem4 (val) {
    ...
    addInfo () {
      ...
      this.$refs.addRef.validate(async (valid) => {
        if (!valid) return
        if (valid) {
          const res = await postData('/exam/examInsert', qs.stringify(this.addFrom))
          console.log(res)
          if (res.code === 200) {
    ...
    del (id) {
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        const res = await delData('/exam/examDelete?eId=' + id)
        if (res.code === 200) {
