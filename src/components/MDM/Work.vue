...
<div>
     <el-card class="box-card" shadow="always">
        ...
        <el-table :data="tableData" style="width: 100%"  tooltip-effect="dark"  @selection-change="handleSelectionChange" border>
          ...
          <el-table-column sortable label="零部件名称" width="110" prop="componentName" align="center"> </el-table-column>
           <el-table-column sortable label="顺序号" width="100" prop="processId" align="center"> </el-table-column>
           ...
           <el-table-column sortable label="工序代号" width="100" prop="processCode" align="center"></el-table-column>
            ...
            <el-table-column sortable label="报工顺序" width="100" prop="sequenceId" align="center"> </el-table-column>
          <el-table-column sortable label="报工要求" width="100" prop="reportParam" align="center"> </el-table-column>
          <el-table-column sortable label="报工类型" width="100" prop="reportType" align="center"> </el-table-column>
           <el-table-column sortable label="定性或着定量" width="120" prop="isQualitative" align="center"></el-table-column>
   ...
   <el-pagination background
    ...
    <!-- 点击打开修改对话框 -->
    <el-dialog title="请修改工艺报工信息" :visible.sync="editDialog" width="70%">
 <el-form :model="editFrom" ref="editRef" :inline="true" class="demo-form-inline" label-width="100px" :rules="editRules">
        <el-form-item label="零部件代号" prop="componentCode">
    ...
    <el-input  v-model="editFrom.processName"></el-input>
     ...
     <el-form-item label="工序代号" prop="processCode">
      ...
      <el-form-item label="工艺类型" prop="carftType" >
     <el-select v-model="editFrom.carftType"  clearable placeholder="请选择零部件代号" @change="getitem2">
          <el-option v-for="item in style" :key="item.carftId" :label="item.carftType" :value="item.carftId"></el-option>
      </el-select>
    </el-form-item>
      <el-form-item label="报工顺序" prop="sequenceId">
      <el-input  v-model="editFrom.sequenceId"></el-input>
    </el-form-item>
     <el-form-item label="报工要求" prop="reportParam">
     <el-input  v-model="editFrom.reportParam"></el-input>
    </el-form-item>
     <el-form-item label="报工类型" prop="reportType">
         <el-select v-model="editFrom.reportType"  clearable placeholder="请选择报工类型" @change="getitem4">
          <el-option v-for="item in styles" :key="item.reportTypeId" :label="item.reportTypeName" :value="item.reportTypeId"></el-option>
      </el-select>
    </el-form-item>
     <el-form-item label="定性或着定量" prop="isQualitative">
     <el-select v-model="editFrom.isQualitative"  clearable placeholder="请选择零部件代号" @change="getitem3">
          <el-option v-for="item in isQ" :key="item.isQualitativeId" :label="item.isQualitativeName" :value="item.isQualitativeId"></el-option>
      </el-select>
    </el-form-item>
  </el-form>
 ...
 <!-- 点击打开增加对话框 -->
    <el-dialog title="请增加工艺报工信息" :visible.sync="addDialog" width="70%">
  <el-form :model="addFrom" ref="addRef" :inline="true" class="demo-form-inline" label-width="100px" :rules="addRules">
        <el-form-item label="零部件代号" prop="componentCode">
    ...
    <el-input  v-model="addFrom.processName"></el-input>
     ...
     <el-form-item label="工序代号" prop="processCode">
      ...
      <el-form-item label="工艺类型" prop="carftType" >
     <el-select v-model="addFrom.carftType"  clearable placeholder="基本批" @change="getitem2">
          <el-option v-for="item in style" :key="item.carftId" :label="item.carftType" :value="item.carftId"></el-option>
      </el-select>
    </el-form-item>
      <el-form-item label="报工顺序" prop="sequenceId">
      <el-input  v-model="addFrom.sequenceId"></el-input>
    </el-form-item>
     <el-form-item label="报工要求" prop="reportParam">
     <el-input  v-model="addFrom.reportParam"></el-input>
    </el-form-item>
     <el-form-item label="报工类型" prop="reportType">
         <el-select v-model="addFrom.reportType"  clearable placeholder="报工" @change="getitem4">
          <el-option v-for="item in styles" :key="item.reportTypeId" :label="item.reportTypeName" :value="item.reportTypeId"></el-option>
      </el-select>
    </el-form-item>
     <el-form-item label="定性或着定量" prop="isQualitative">
     <el-select v-model="addFrom.isQualitative"  clearable placeholder="请选择零部件代号" @change="getitem3">
          <el-option v-for="item in isQ" :key="item.isQualitativeId" :label="item.isQualitativeName" :value="item.isQualitativeId"></el-option>
      </el-select>
    </el-form-item>
  </el-form>
...
export default {
  ...
  data () {
    return {
      ...
      addRules: {
        ...
        sequenceId: [
          { required: true, message: '只能输入报工顺序，不能为空', trigger: 'blur' }
        ],
        reportParam: [
          { required: true, message: '只能输入报工要求,不能为空', trigger: 'blur' }
        ]
      },
      editRules: {
        componentName: [
  ...
  methods: {
    // 获取表格数据
    async getWork () {
      const res = await getData('/exam/reportSelect', this.queryWork)
      if (res.code === 200) {
    ...
    // 搜索按钮
    async getGo () {
      const res = await getData('/exam/reportSelectLike', this.form)
      if (res.code === 200) {
    ...
    editInfo () {
      ...
      this.$refs.editRef.validate(async (valid) => {
        if (!valid) return
        if (valid) {
          const res = await postData('/exam/reportUpdate', qs.stringify(this.editFrom))
          console.log(res)
          if (res.code === 200) {
    ...
    // 获取下拉框数据
    async getSelects () {
      const res = await getData('/exam/reportSelectLists')
      if (res.code === 200) {
        this.code = res.data.componentCode
        this.style = res.data.carftType
        this.styles = res.data.reportType
        this.isQ = res.data.isQualtatative
      }
    },
    getitems (val) {
      this.code.find((item) => {
    ...
    async  getProcess1 () {
      const res = await getData('/exam/reportProcessIds?componentCode=' + this.editFrom.componentCode)
      if (res.code === 200) {
    ...
    // 获取下拉框数据
    async getSelect () {
      const res = await getData('/exam/reportSelectList')
      if (res.code === 200) {
        this.code = res.data.componentCode
        this.style = res.data.carftType
        this.styles = res.data.reportType
    ...
    async  getProcess () {
      const res = await getData('/exam/reportProcessId?componentCode=' + this.addFrom.componentCode)
      if (res.code === 200) {
        this.num = res.data.processId
      } else {
      }
    ...
    getitem3 (val) {
      this.isQ.find(item => item.isQualitativeId === val)
    },
    getitem4 (val) {
      this.styles.find(item => item.reportTypeId === val)
    },
    // 打开增加按钮对话框
    addButton () {
      this.addDialog = true
    ...
    addInfo () {
      ...
      this.$refs.addRef.validate(async (valid) => {
        if (!valid) return
        if (valid) {
          const res = await postData('/exam/reportInsert', qs.stringify(this.addFrom))
          console.log(res)
          if (res.code === 200) {
    ...
    del (id) {
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        const res = await delData('/exam/reportDelete?eId=' + id)
        if (res.code === 200) {
